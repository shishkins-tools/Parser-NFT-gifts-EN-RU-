...
def save_to_file(links):
    try:
        with open(FILE_NAME, "w", encoding="utf-8") as f:
            f.write("\n".join(links))
        messagebox.showinfo("Success!", f"Links successfully written to: {FILE_NAME}")
...
def generate_all():
    ...
    save_to_file(links)
root = tk.Tk()
root.title("(EN) NFT Link Generator")
root.geometry("300x200")
label = tk.Label(root, text=f"In the database: {len(NFT_COLLECTIONS)} NFT collections.", pady=20)
label.pack()
btn_ten = tk.Button(root, text="10 random NFT", command=generate_ten, width=25, height=2)
btn_ten.pack(pady=5)
btn_all = tk.Button(root, text="One of all", command=generate_all, width=25, height=2)
btn_all.pack(pady=5)
root.mainloop()